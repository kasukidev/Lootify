...
@Getter
public abstract class AbstractMongoCacheRepository<K, V> extends AbstractMongoRepository<K>
    implements ICacheRepository<K, V> {

  private final CaffieneCacheRepository<K,V> cacheRepository;

  protected AbstractMongoCacheRepository(MongoCollection<Document> collection, Cache<K, V> cache) {
    super(collection);
    this.cacheRepository = new CaffieneCacheRepository<>(cache);
  }

  protected AbstractMongoCacheRepository(MongoCollection<Document> collection) {
    super(collection);
